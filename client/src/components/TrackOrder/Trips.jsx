...
export default function Trips({ timeslot }) {
  ...
  // Fetch trips whenever timeslot changes
  useEffect(() => {
    if (!timeslot) return;
  ...
  const LoadingDots = () => {
    return (
      <span className="inline-flex items-center">
        ...
        ></span>
        <span
          className="w-2 h-2 rounded-full bg-amber-500 dark:bg-amber-400 mx-1"
  ...
  const TableComponent = ({ data, title }) => (
    <div
      className="w-full mb-8 transform transition-all duration-500 hover:scale-[1.01]"
      style={{ animation: "fadeIn 0.5s ease-out forwards" }}
    >
      <h5
        className={`text-xl sm:text-2xl font-bold mb-4 bg-gradient-to-r ${getGradientColor(
          title
        )} bg-clip-text text-transparent flex items-center gap-2`}
      >
        {vehicleTypes.find((t) => t.id === title)?.icon}{" "}
        {vehicleTypes.find((t) => t.id === title)?.label || title} Trips
      </h5>
          ...
          <table className="min-w-full divide-y divide-amber-200 dark:divide-amber-700/30">
            <thead>
              <tr className={`bg-gradient-to-r ${getGradientColor(title)}`}>
                <th className="py-4 px-3 text-sm font-semibold text-white border-r border-amber-400/20">
                  No.
                </th>
                <th className="py-4 px-3 text-sm font-semibold text-white border-r border-amber-400/20">
                  Shipments
                ...
                <th className="py-4 px-3 text-sm font-semibold text-white w-24">
                  Track
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-amber-100 dark:divide-amber-700/30">
              {data.length > 0 ? (
                data.map((trip, index) => (
                  ...
                  >
                    <td className="py-3 px-3 text-sm text-center text-gray-700 dark:text-gray-200 border-r border-amber-200 dark:border-amber-700/30 group-hover:font-semibold">
                      {index + 1}
                    </td>
                    <td className="py-3 px-3 text-sm text-center text-gray-700 dark:text-gray-200 border-r border-amber-200 dark:border-amber-700/30 group-hover:font-semibold">
                      {trip["Total Shipments"]}
                    </td>
                    <td className="py-3 px-3 text-sm text-center text-gray-700 dark:text-gray-200 border-r border-amber-200 dark:border-amber-700/30 group-hover:font-semibold">
                      {trip["Shipments Delivered"]}
                    </td>
                    ...
                    <td className="py-3 px-3 text-sm text-center text-gray-700 dark:text-gray-200 border-r border-amber-200 dark:border-amber-700/30 group-hover:font-semibold">
                      {trip["Time Utilization"]}
                    </td>
                    <td className="py-3 px-3 text-sm text-center text-gray-700 dark:text-gray-200 border-r border-amber-200 dark:border-amber-700/30 group-hover:font-semibold">
                      {trip["COV_UTI (Distance Utilization)"]}
                    </td>
                    <td className="py-3 px-3 text-center">
                      ...
                      >
                        <span
                          className={`absolute inset-0 bg-gradient-to-r ${getGradientColor(
                            title
                          )} transition-all duration-300 group-hover/btn:opacity-90`}
                        ></span>
                ...
                <tr>
                  <td
                    colSpan="10"
                    className="py-8 text-center text-amber-600 dark:text-amber-400 animate-pulse"
                  >
                    No trips available
                  </td>
                </tr>
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-amber-50 to-orange-50 dark:from-gray-900 dark:to-gray-800 transition-all duration-500 p-4 sm:p-6 lg:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-wrap justify-center gap-4 mb-8">
          {vehicleTypes.map(({ id, label, icon, gradient }) => (
            ...
            >
              {selectedVehicleTypes.includes(id) && (
                <span
                  className={`absolute inset-0 bg-gradient-to-r ${gradient} transition-opacity`}
                ></span>
              )}
              <span
                className={`
        ...
        {error && (
          <div
            className="mb-6 p-4 bg-orange-50 dark:bg-orange-900/30 border-l-4 border-orange-500 rounded-r-lg"
            style={{
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-16 animate-pulse">
            <div className="relative w-24 h-24 mb-6">
              <div
                className="absolute inset-0 rounded-full border-4 border-amber-200 dark:border-amber-700/30 border-t-amber-500 dark:border-t-amber-500"
                style={{ animation: "spin 1.5s linear infinite" }}
              ></div>
              <div
                className="absolute inset-4 rounded-full border-4 border-orange-200 dark:border-orange-700/30 border-b-orange-500 dark:border-b-orange-500"
                style={{ animation: "spin 1.5s linear infinite reverse" }}
              ></div>
            </div>
            <p className="text-lg font-medium text-amber-600 dark:text-amber-400 flex items-center gap-2">
              Please wait, loading trip data
              <LoadingDots />
            </p>
          </div>
        ) : (
            ...
            {selectedVehicleTypes.length === 0 ? (
              ...
              selectedVehicleTypes.map((vehicleType) => (
                <TableComponent
                  key={vehicleType}
                  data={trips.filter(
                    (trip) => trip["Vehicle Type"] === vehicleType
                  )}
                  title={vehicleType}
                />
              ))
