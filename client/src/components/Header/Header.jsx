...
export default function Header() {
  ...
  const linkClass = ({ isActive }) =>
    `block py-2 pr-4 pl-3 duration-200 ${
      isActive
        ? "text-orange-700 dark:text-orange-400 underline underline-offset-4"
        : "text-gray-700 dark:text-gray-300"
    } hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-orange-700 
    lg:p-0 hover:decoration-2`;

  return (
    <header className="sticky top-0 z-50 shadow">
      <nav className="bg-white dark:bg-gray-900 border-gray-200 px-4 lg:px-6 py-2.5">
        <div className="flex flex-wrap sm:flex-nowrap items-center justify-between max-w-screen-xl mx-auto gap-2 sm:gap-4">
          {/* Logo */}
              ...
              {menuItems.map(({ name, path }) => (
                <li key={path}>
                  <NavLink
                    to={path}
                    className={({ isActive }) =>
                      `block py-2 px-2 sm:text-sm md:text-base duration-200 ${
                        isActive
                          ? "text-orange-700 dark:text-orange-400 underline underline-offset-4"
                          : "text-gray-700 dark:text-gray-300"
                      } hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-orange-700 sm:p-0 hover:decoration-2`
                    }
                  >
                    {name}
                  </NavLink>
          ...
          <div className="flex items-center sm:gap-1 md:gap-2 sm:order-2 flex-nowrap">
            ...
            >
              <FontAwesomeIcon
                ...
                } hover:text-blue-500 fa-lg sm:fa-xl transition-all`}
              />
            </button>
