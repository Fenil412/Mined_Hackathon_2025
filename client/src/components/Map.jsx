...
const Map = () => {
  ...
  // Fetch individual trip details and map when timeslot or index changes
  useEffect(() => {
    const fetchTripAndMap = async () => {
      if (!timeslot || index === undefined || index === null) {
      ...
      try {
        ...
        // Fetch trips data again if needed or use cached trips in state
        ...
        // Fetch the map HTML for the selected trip
        const mapResponse = await axios.get(
          `${import.meta.env.VITE_API_URL}/api/map`,
          { params: { timeslot, index } }
        );
                ...
                <div className="bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 rounded-lg p-3 flex flex-col items-center">
                  ...
                  <span className="text-xl font-bold text-amber-600 dark:text-amber-400 flex items-center">
                    {tripDetails["Vehicle Type"] && (
                      <>
                        <span className="mr-2">
                          {getVehicleInfo(tripDetails["Vehicle Type"]).icon}
                        </span>
